                            ...
                            if db_update_result[0] > 0 and (db_update_result[1] >= cur_min or cur_min == 59):
                                ...
                                if analysis_data_df_min is None or analysis_data_df_day is None:
                                    sg.g_logger.write_log(f"{stock_code} analysis_data_df_min or day is None", log_lv=3,
                                                          is_con_print=False)
                                    continue
                                limt_min_amount = ((analysis_data_amount_min * 3) // 5) * sg.g_one_day_data_amount
                                limt_day_amount = (analysis_data_amount_day * 3) // 5
                                ...
                                macd_stoch_data_df_min = macd_stoch_data_df_min.iloc[-1]
                                # ===========================
                                is_buy = sg.g_ets.is_buy_sell(slow_d_day=macd_stoch_data_df_day['slow_d'],
                                                               df_macd_min=macd_stoch_data_df_min,
                                                               ave_min=macdhist_ave_min)
                                if is_buy is True:
                                    # ============== 산다 ================
        ...
        while True:
            ...
            for bought_stock in bought_list:
                ...
                if True:
                    ...
                    analysis_data_df_day = sg.g_market_db.get_past_stock_price(stock_code, analysis_data_amount_day,
                                                                               chart_type="D")
                    if analysis_data_df_min is None or analysis_data_df_day is None:
                        sg.g_logger.write_log(f"{stock_code} analysis_data_df_min or day is None", log_lv=5)
                        continue
                    ...
                    if is_sell is False:
                        ...
                        if is_sell_success is True:
                            ...
                            sg.g_logger.write_log(f"매도 했습니다.\r\n"
                                                  f"stock_code = {stock_code}\r\n"
                                                  f"1주 가격 = {current_price}\r\n"
                                                  f"이제 {max_buy_stock_count - bought_count}개 살 수 있습니다.", log_lv=2,
                                                  is_slacker=True)
